...
if [[ $# -lt 12 ]];then
    echo "usage: sh $0 -SN -dataDir -registJson -speciesName -tissueType -outDir -imageRecordFile -imageCompressedFile -doCellBin -threads -dockerUrl
    -SN : sample id
...
if [[ -f $imageTarFile ]] && [[ -f $iprFile ]]  && [[ $doCell == "Y" ]]; then
    ...
    /usr/bin/time -v docker run -e HDF5_USE_FILE_LOCKING=FALSE --rm -i -v $outDir:$outDir -v $imgTarDIR:$imgTarDIR -v $iprDIR:$iprDIR -v $dataDir:$dataDir ${dockerUrl} /bin/bash manualRegister \
			-i ${tifDir} \
    ...
    /usr/bin/time -v docker run -e HDF5_USE_FILE_LOCKING=FALSE --rm -i -v $outDir:$outDir -v $imgTarDIR:$imgTarDIR -v $iprDIR:$iprDIR -v $dataDir:$dataDir ${dockerUrl} /bin/bash imageTools ipr2img \
            -i ${imageTarFile} \
            -c ${out_iprFile} \
            -d tissue cell \
            -r True \
...
if [[ -f $imageTarFile ]] && [[ -f $iprFile ]]; then
    ...
    export HDF5_USE_FILE_LOCKING=FALSE
    /usr/bin/time -v docker run -e HDF5_USE_FILE_LOCKING=FALSE --rm -i -v $outDir:$outDir -v $dataDir:$dataDir ${dockerUrl} /bin/bash tissueCut \
        -i ${geneExp} \
    ...
    /usr/bin/time -v docker run -e HDF5_USE_FILE_LOCKING=FALSE --rm -i -v $outDir:$outDir -v $dataDir:$dataDir ${dockerUrl} /bin/bash cellCut bgef \
        -i ${result_04tissuecut}/${SN}.tissue.gef \
        -o ${result_04tissuecut}/${SN}.${nucleusLayer}.gef \
        -O Transcriptomics \
        -b 1,5,10,20,50,100,150,200
    
    for i in `docker run -e HDF5_USE_FILE_LOCKING=FALSE --rm -i -v $outDir:$outDir -v $dataDir:$dataDir ${dockerUrl} /bin/bash h5dump -n ${result_03register}/${SN}.reregist.ipr | grep 'Labeling/' | grep -v 'Labeling/.*/Canvas'|grep -v 'Labeling/.*/Element'|awk '{print$2}'`;
    do
    label=`basename $i`
    ...
    export HDF5_USE_FILE_LOCKING=FALSE
    /usr/bin/time -v docker run -e HDF5_USE_FILE_LOCKING=FALSE --rm -i -v $outDir:$outDir -v $refDIR:$refDIR -v $dataDir:$dataDir ${dockerUrl} /bin/bash tissueCut \
        -i ${geneExp} \
        --dnbfile ${barcodeReadsCounts} \
        --sn ${SN} \
        -O Transcriptomics \
...
export HDF5_USE_FILE_LOCKING=FALSE
/usr/bin/time -v docker run -e HDF5_USE_FILE_LOCKING=FALSE --rm -i -v $outDir:$outDir -v $refDIR:$refDIR -v $dataDir:$dataDir ${dockerUrl} /bin/bash cellCut bgef \
    -i ${geneExp} \
    -o ${result_04tissuecut}/${SN}.gef \
    -O Transcriptomics
	
# Complete raw labeled tissue GEF to visual GEF	
for i in `docker run -e HDF5_USE_FILE_LOCKING=FALSE --rm -i -v $outDir:$outDir -v $refDIR:$refDIR -v $dataDir:$dataDir ${dockerUrl} /bin/bash h5dump -n ${result_03register}/${SN}.reregist.ipr | grep 'Labeling/' | grep -v 'Labeling/.*/Canvas'|grep -v 'Labeling/.*/Element'|awk '{print$2}'`;
do
label=`basename $i`
export HDF5_USE_FILE_LOCKING=FALSE
/usr/bin/time -v docker run -e HDF5_USE_FILE_LOCKING=FALSE --rm -i -v $outDir:$outDir -v $refDIR:$refDIR -v $dataDir:$dataDir ${dockerUrl} /bin/bash cellCut bgef \
...
/usr/bin/time -v docker run -e HDF5_USE_FILE_LOCKING=FALSE --rm -i -v $outDir:$outDir -v $refDIR:$refDIR -v $dataDir:$dataDir ${dockerUrl} /bin/bash cellCut view \
    -s ${SN} \
...
binSize=200
resolution=1.0
/usr/bin/time -v docker run -e HDF5_USE_FILE_LOCKING=FALSE --rm -i -v $outDir:$outDir -v $dataDir:$dataDir ${dockerUrl} /bin/bash spatialCluster \
    -i ${result_04tissuecut}/${SN}.tissue.gef \
...
if [[ $doCell == 'Y' ]]; then
    ...
    /usr/bin/time -v docker run -e HDF5_USE_FILE_LOCKING=FALSE --rm -i -v $outDir:$outDir -v $dataDir:$dataDir ${dockerUrl} /bin/bash cellCut cgef \
        -i ${geneExp} \
        -m ${nucleusMask} \
    ...
    export HDF5_USE_FILE_LOCKING=FALSE
    /usr/bin/time -v docker run -e HDF5_USE_FILE_LOCKING=FALSE --rm -i -v $outDir:$outDir -v $dataDir:$dataDir ${dockerUrl} /bin/bash cellCorrect \
    ...
    /usr/bin/time -v docker run -e HDF5_USE_FILE_LOCKING=FALSE --rm -i -v $outDir:$outDir -v $dataDir:$dataDir ${dockerUrl} /bin/bash imageTools img2rpi \
        -i ${cellCorrectMask} \
    ...
    export NUMBA_CACHE_DIR=${outDir}/tmp
    export MPLCONFIGDIR=${outDir}/tmp
    /usr/bin/time -v docker run -e HDF5_USE_FILE_LOCKING=FALSE --rm -i -v $outDir:$outDir -v $dataDir:$dataDir ${dockerUrl} /bin/bash cellCluster \
  ...
  /usr/bin/time -v docker run -e HDF5_USE_FILE_LOCKING=FALSE --rm -i -v $outDir:$outDir -v $dataDir:$dataDir ${dockerUrl} /bin/bash cellChunk \
...
/usr/bin/time -v docker run -e HDF5_USE_FILE_LOCKING=FALSE --rm -i -v $outDir:$outDir -v $dataDir:$dataDir ${dockerUrl} /bin/bash saturation \
    -i ${saturationFile} \
...
if [[ -n ${out_iprFile} ]] && [[ -e ${out_iprFile} ]] && [[ $doCell == 'Y' ]]; then
    /usr/bin/time -v docker run -e HDF5_USE_FILE_LOCKING=FALSE --rm -i -v $outDir:$outDir -v $dataDir:$dataDir ${dockerUrl} /bin/bash report \
        -m ${bcStatStr} \
        -a ${bcFinalOutStr} \
